...
import { ArrowUpRight, CircleDot, Clock3, Lock, Sparkles } from 'lucide-react';
...
export function CaseSelectionCard({ caseData, caseKey, index, status = 'new', isLocked = false, lockHint }) {
  ...
  return (
    <motion.article
      ...
      transition={{ duration: 0.5, delay: index * 0.1 }}
      whileHover={{ y: -8 }}
      className="clip-corner group relative flex h-full flex-col panel-surface p-7 shadow-panel transition-colors duration-300 hover:border-crimson/45"
    >
      <span aria-hidden="true" className="absolute inset-x-0 top-0 h-[3px] scale-x-0 bg-gradient-to-r from-transparent via-crimson-bright to-transparent transition-transform duration-500 group-hover:scale-x-100" />
      {body}
      <Link
        to={`/case/${caseKey}`}
