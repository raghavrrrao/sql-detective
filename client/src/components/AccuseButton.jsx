...
export function AccuseButton({ onOpen }) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-20 flex flex-col items-end gap-2.5 sm:bottom-6 sm:right-6">
      ...
      <p
        ...
        role="status"
        aria-live="polite"
        className={`clip-corner-sm max-w-[10.5rem] border px-3 py-2 text-right text-xs leading-5 sm:max-w-[15rem] ${
          readiness.isReady
      ...
      <button
        ...
        className="clip-corner-sm inline-flex items-center gap-2.5 border border-gold-bright/70 bg-gold px-4 py-3 font-display text-sm sm:px-5 sm:py-3.5 font-medium uppercase tracking-[0.16em] text-ink shadow-glow transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:border-white/12 disabled:bg-charcoal-light disabled:text-bone-dim disabled:shadow-none disabled:hover:translate-y-0"
      >
