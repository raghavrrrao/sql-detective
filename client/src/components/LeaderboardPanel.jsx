  ...
  return (
    <section aria-label="Leaderboard" className={`clip-corner panel-surface shadow-panel ${className}`}>
      <header className="flex items-center gap-3 border-b border-white/10 bg-white/[0.03] px-5 py-4">
        ...
        <h2 className="font-display text-base font-semibold uppercase tracking-[0.18em] text-bone">Leaderboard</h2>
              ...
              <tr>
                {['#', 'Detective', 'Case', 'Time', 'Hints', 'Score'].map((heading) => (
                  <th
                    key={heading}
                    scope="col"
                    className="whitespace-nowrap border-b border-gold/25 px-4 py-3 font-display text-xs font-semibold uppercase tracking-[0.16em] text-gold-bright"
                  >
                    {heading}
                  </th>
                ))}
            ...
            <tbody>
              {rows.map((entry, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-semibold text-bone">{entry.name}</td>
                  ...
                  <td className="px-4 py-3 font-mono font-semibold text-gold-bright">{entry.score}</td>
                </tr>
              ))}
            </tbody>
          </table>
